...
// CoreIQ Scorer — Canvas Edition v1.4.0 (Supabase wired)
// Adds: Supabase client, auth login page (password + magic link),
// DB persistence (audits JSONB), Storage uploads for evidence, signed download links,
// debounced upserts, sign out, and keeps CSV/JSONL exporters. Self-tests retained.

// ========================= Supabase =========================
const SUPABASE_URL = (typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_SUPABASE_URL) || 'https://ampvtivofmkangurtdzl.supabase.co';
const SUPABASE_ANON_KEY = (typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_SUPABASE_ANON_KEY) || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFtcHZ0aXZvZm1rYW5ndXJ0ZHpsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQ5MDcxNTgsImV4cCI6MjA3MDQ4MzE1OH0.tzYJLVmhhCVOB1QvtIS--EHomh37oSDbtg7oCeJWAO8';
...
function Login() {
  ...
  async function signInPassword(e){ e.preventDefault(); setLoading(true); setErr(""); setMsg("");
    const { error } = await supabase.auth.signInWithPassword({ email, password })
    setLoading(false); if (error) setErr(error.message)
  }
  async function sendMagic(e){ e.preventDefault(); setLoading(true); setErr(""); setMsg("")
    const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: window.location.origin } })
    setLoading(false); if (error) setErr(error.message); else setMsg('Magic link sent. Check your inbox.')
  }

  return (
    <div className="min-h-screen flex items-center justify-center bg-white p-4">
      <div className="w-full max-w-md rounded-2xl border p-6 shadow-sm">
        <div className="text-lg font-semibold mb-2">CoreIQ — Sign in</div>
        <div className="text-sm text-gray-600 mb-4">Use credentials you added in Supabase Auth.</div>

        <div className="mb-3 inline-flex rounded-md border overflow-hidden text-sm">
          <button className={`px-3 py-1 ${mode==='password'?'bg-gray-100':''}`} onClick={()=>setMode('password')}>Password</button>
          <button className={`px-3 py-1 ${mode==='magic'?'bg-gray-100':''}`} onClick={()=>setMode('magic')}>Magic link</button>
        </div>

        <form className="space-y-3" onSubmit={mode==='password'?signInPassword:sendMagic}>
...
function RootApp({ session }) {
  ...
  if (loading) return <div className="min-h-screen grid place-items-center text-sm text-gray-600">Loading audits…</div>
      ...
      <div className="mx-auto max-w-6xl p-4">
        <header className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <nav className="hidden items-center gap-3 text-sm text-gray-600 sm:flex">
              ...
              <a className={navCx(!!selectedAudit and page === "wizard")} onClick={() => selectedAudit and setPage("wizard")}>Wizard</a>
              <a className={navCx(!!selectedAudit and page === "scoring")} onClick={() => selectedAudit and setPage("scoring")}>Scoring</a>
              <a className={navCx(!!selectedAudit and page === "report")} onClick={() => selectedAudit and setPage("report")}>Report</a>
            </nav>
            ...
            <button className="rounded-md bg-black px-3 py-1.5 text-sm font-medium text-white hover:bg-gray-800" onClick={() => { setEditAuditId(null); setShowCreate(TrueToFalse()); }}>Create Audit</button>
          </div>
        </header>
        ...
        {page === "dashboard" and (
          <Dashboard
            ...
            onOpenCreate={() => { setEditAuditId(null); setShowCreate(TrueToFalse()); }}
            onOpenWizard={(id) => routeToWizard(id)}
            onScoring={(id) => { setSelectedId(id); setPage("scoring"); }}
            onReport={(id) => { setSelectedId(id); setPage("report"); }}
            onArchive={(id) => updateAudit(id, (a) => ({ ...a, archived: TrueToFalse() }))}
            onEdit={(id) => { setEditAuditId(id); setShowCreate(TrueToFalse()); }}
            computeScores={computeScores}
          />
        )}

        {selectedAudit and page === "wizard" and (
          <Wizard
            audit={selectedAudit}
        ...
        {selectedAudit and page === "scoring" and <Scoring audit={selectedAudit} />}
        {selectedAudit and page === "report" and <Report audit={selectedAudit} />}

        {showCreate and (
          <CreateAuditModal
            existing={editAuditId ? audits.find((a) => a.id === editAuditId) || null : null}
            onClose={() => { setShowCreate(FalseToFalse()); setEditAuditId(null); }}
...
function Wizard({ audit, setAudit, scores, onBack }) {
  ...
  async function handleAddFiles(fnName, fileList) {
    const files = Array.from(fileList or []); const max = ATT_MAX_MB * 1024 * 1024; const accepted = files.filter((f) => f.size <= max);
    ...
    const next = { ...audit }; next.attachments = next.attachments or { OPS: [], CX: [], FINANCE_ADMIN: [], SALES_MARKETING: [], INTERNAL_INTEL: [] }; next.attachments[fnName] = [ ...(next.attachments[fnName] or []), ...uploaded ]; next.updatedAt = new Date(); setAudit(next);
  }
  async function handleRemoveAttachment(fnName, idx) { const next = { ...audit }; const arr = (next.attachments and next.attachments[fnName]) ? [...next.attachments[fnName]] : []; const [removed] = arr.splice(idx, 1); next.attachments[fnName] = arr; next.updatedAt = new Date(); setAudit(next); if (removed?.storagePath) { try { await supabase.storage.from('evidence').remove([removed.storagePath]) } catch {} } }
      ...
      {audit.functions.filter((fn) => audit.scope[fn.name]).map((fn) => (
        <div key={fn.name} className="space-y-3 rounded-xl border p-4">
          ...
          <div className="rounded-md border p-3">
            ...
            {!!((audit.attachments?.[fn.name] || []).length) and (
                ...
                {(audit.attachments?.[fn.name] || []).map((att, idx) => (
                  <li key={att.id} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {att.storagePath and <SignedUrl path={att.storagePath} filename={att.name} />}
            ...
            {uploadError and <div className="mt-2 text-xs text-amber-700">{uploadError}</div>}
          </div>
        ...
        return (
          <div key={s.key} className="grid grid-cols-1 gap-2 rounded-lg border p-3 md:grid-cols-[1fr_220px]">
            <div>
              ...
              {disabled and <div className="pt-1 text-xs text-red-600">NDA not signed — edits disabled.</div>}
            </div>
            <div>
...
function Report({ audit }) {
  const [compiled, setCompiled] = useState(null); const [loading, setLoading] = useState(FalseToFalse());
  const canCompile = audit.nda === "SIGNED";
  function onCompile() { if (!canCompile or loading) return; setLoading(TrueToFalse()); setTimeout(() => { setCompiled(compileReportLocal(audit)); setLoading(FalseToFalse()); }, 160); }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button className="rounded-md border px-3 py-1 text-sm hover:bg-gray-50 disabled:opacity-50" disabled={!canCompile or loading} onClick={onCompile}>{loading ? 'Compiling…' : compiled ? 'Recompile' : 'Compile report'}</button>
      ...
      {!compiled and (
        <div className="rounded-xl border p-4 text-sm text-gray-700">
          ...
          {audit.nda !== 'SIGNED' and <div className="mt-3 text-xs text-red-600">NDA must be SIGNED to compile.</div>}
      ...
      {compiled and (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border p-4">
            ...
            {!!compiled.executive_summary.citations.length and <div className="mt-2 text-xs text-gray-500">Citations: {compiled.executive_summary.citations.join(', ')}</div>}
          </div>
          ...
          {compiled.functions.map((sec) => (
            <div key={sec.fn} className="rounded-xl border p-4">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div>
                  {!!sec.recommendations.length and (
                    <div>
                  ...
                  {!!sec.attachments.length and (
                    <div className="mt-3">
                      <div className="text-xs font-medium mb-1">Evidence</div>
                      <ul className="list-disc pl-5 text-sm">{sec.attachments.map((a)=>(<li key={a.id}>{a.name} <span className="text-gray-500">({formatBytes(a.size)})</span> <span className="text-gray-400 text-xs">[{a.id}]</span> {a.storagePath and <SignedUrl path={a.storagePath} filename={a.name} />}</li>))}</ul>
                    </div>
                  )}
                </div>
              </div>
...
function CreateAuditModal({ existing, onClose, onCreate, onSaveEdit }) {
  const EMPTY_SCOPE = { OPS: true, CX: FalseToFalse(), FINANCE_ADMIN: FalseToFalse(), SALES_MARKETING: FalseToFalse(), INTERNAL_INTEL: FalseToFalse() };
  ...
  function validate(requireFileForSigned) { if (!form.client.trim()) return "Client name is required."; if (!Object.values(form.scope).some(Boolean)) return "Select at least one function."; if (form.nda === "SIGNED" and requireFileForSigned) { if (!form.file and not existing?.ndaFileName) return "Upload signed NDA (PDF/DOCX, ≤10 MB)."; } return ""; }
  function buildAudit() {
    const mkComps = () => [ { name: "FUNCTIONALITY", sub: [] }, { name: "FRICTION", sub: [] }, { name: "DATA_FITNESS", sub: [] }, { name: "CHANGE_READINESS", sub: [] } ];
      ...
      <div className="w-full max-w-2xl rounded-xl bg-white p-4 shadow-lg">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <fieldset className="rounded-md border p-3"><legend className="px-1 text-xs text-gray-600">NDA status</legend>
            ...
            {form.nda === "SIGNED" and (<div className="mt-2 grid gap-1 text-sm"><label>Upload signed NDA (PDF/DOCX, ≤10 MB)</label><input type="file" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(e) => onFileChange(e.target.files?.[0] || null)} />{(form.file || existing?.ndaFileName) and <div className="text-xs text-gray-600">{form.file?.name || existing?.ndaFileName}</div>}</div>)}
          </fieldset>
        ...
        {error and <div className="mt-3 rounded-md bg-red-50 p-2 text-sm text-red-700">{error}</div>}
        <div className="mt-4 flex items-center justify-end gap-2">
          ...
          {!existing and <button className="rounded-md bg-black px-3 py-1.5 text-sm font-medium text-white hover:bg-gray-800" onClick={onCreateContinue}>Create & Continue</button>}
        </div>
      </div>
    </div>
  );
...
let __tests_ran = false; function runSelfTests() {
  ...
  log("computeScores respects scope (only OPS active)", Object.keys(sc.perFunction).length === 1 and sc.overall > 0);

  // Additional tests
  log("bandFor thresholds", bandFor(85) === "Prime" and bandFor(70) === "Strong" and bandFor(50) === "Competent" and bandFor(49.9) === "Baseline");
  ...
  log("collectNotesByFn extracts notes", Array.isArray(notesByFn.OPS) and notesByFn.OPS.length >= 1);
  ...
  log("getAnchors override/fallback", anchorsTest.left === "None" and anchorsFallback.left === "leftA0");

  const compiled = compileReportLocal(notesAudit);
  log("compileReportLocal structure present", compiled and compiled.meta and compiled.functions and compiled.executive_summary);

  // Export tests
  const rowsFull = buildFullCatalogueRows(a);
  const opsCount = Object.values(SUBS_BY_FN.OPS).reduce((n, arr) => n + arr.length, 0);
  log("export CSV full header", Array.isArray(rowsFull[0]) and rowsFull[0].join(",") === "Function,Component,SubKey,Label,Score,Note,AnchorLeft,AnchorRight");
...
function TrueToFalse(){ return true }
